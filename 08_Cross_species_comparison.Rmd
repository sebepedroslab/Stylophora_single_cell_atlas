...
```{r}
plotdir <- file.path(outdir,"vioin")
dir.create(outdir,showWarnings=FALSE)
```
...
```{r pairwise_links}
circos_files <- c(
  "cross_species/circos/circos_Spis_Nvec_ct_kld_0.98.RDS",
  "cross_species/circos/circos_Spis_Xesp_ct_kld_0.98.RDS",
  "cross_species/circos/circos_Spis_Hvul_ct_kld_0.98.RDS"
)
...
cts_list <- c(ngl_list,lwd_list)
```

Get common genes for groups of cell types

```{r}
feature_in_thrs=1.2; feature_out_thrs=1.8,
gene_list <- lapply(names(cts_list), function(i) {
  ...
  cgenes <- commonGenes(
    mc_fp, cols=ct, feature_in_thrs=1.2, feature_out_thrs=1.8,
...
fwrite(
  orthdt, 
  file.path(outdir,sprintf("selected.neuro.gland.orthologs.fc%s.fcout%s.violin.tsv",feature_in_thrs,feature_out_thrs)),
  sep="\t",col.names=TRUE
)
```
...
#gp=egg::ggarrange(plots=plot_list_final, ncol=1, left="gene expression fold change")

# single plot
pdf(
  file.path(plotdir,sprintf("pairwise.ct.expression.fc%s.fcout%s.violin.tsv",feature_in_thrs,feature_out_thrs)),
...
# old_csps_mcfp <- read.table("../../cross_sp/gene_expression/Spis_Nvec_Xesp_Hvul_broad_cell_type_gene_FC")
# old_var_genes <- names(which(apply(old_csps_mcfp,1,function(x) max(x)>var_genes_fc))) # 1192
# 
# library(eulerr)
# plot(euler(list(old=old_var_genes,new=var_genes)), quantities=TRUE)
```
...
tree_file=file.path(outdir,"species_vargenes1.8_coocaveragep0.75hclusth0.75,0.95_politomies0.10.beast")
tree <- as.phylo(read.beast(tree_file)); plot(tree,tip.color=ctcol[tree$tip.label])
treeColorTips(
  tree_file=tree_file, ctcol=ctcol,
  out_file=file.path(outdir,"broad_cell_type_species_vargenes1.8_coocaveragep0.75hclusth0.75,0.95_politomies0.10.beast")
)
